...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # for example, the A0 level will iterate through all possible actions for the problem and add a PgNode_a to a_
        #   levels[0] set iff all prerequisite literals for the action hold in S0.  This can be accomplished by testing
        ...
        for action in self.all_actions:
            ...
            if self.s_levels[level] >= a_node.prenodes and a_node.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(a_node)

        # connect the S and A nodes
    ...
    def add_literal_level(self, level):
        ...
        for a_node in self.a_levels[level - 1]:
            self.s_levels[level] |= a_node.effnodes  # union of sets
    ...
    #         i = 0
    #         while i < len(self.s_levels):
    #             for PgNode_s in self.s_levels[i]:
    ...
    def level_cost(self, g):
        for level_number, s_node_set in enumerate(self.s_levels):
            level_literal_set = set(n.symbol for n in s_node_set if n.is_pos)
