...
def get_results():
    ...
    problems = [PROBLEMS[i-1] for i in map(int, [1, 2, 3])]
    searches = [SEARCHES[i-1] for i in map(int, [8, 9, 10])]

    results = []
    for pname, p in problems:
        for sname, s, h in searches:

            sys.stdout.write("Running {0} with heuristic {2} on {1}...\n".format(sname, pname, h))
            _p = p()
            _h = None if not h else getattr(_p, h)

            start = timer()

            ip = PrintableProblem(_p)
            node = s(ip, _h)

            end = timer()

            elapsed_time = end - start
            result = [ip.succs, ip.goal_tests, ip.states, len(node.solution()), elapsed_time]
